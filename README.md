...
- **`pip install -e .[test]` fails with `Multiple top-level packages discovered`**: use the latest repo version that uses setuptools custom package discovery (`find` include/exclude) in `pyproject.toml`, then rerun install.
